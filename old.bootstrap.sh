...
git_repository_tarball="${git_repository%.*}/tarball/master"
...
# Get the files using curl or wget
fn_download() {
  local cmd

  if fn_cmd_exists "curl"; then
    cmd="curl -#L"
  elif fn_cmd_exists "wget"; then
    cmd="wget --no-check-certificate -O -"
  fi

  if [ -z "$cmd" ]; then
    fn_print_error "No curl or wget available. Aborting."
    exit 1
  else
    mkdir -p "$2"
    eval "$cmd $1 | tar -xzv -C $2 --strip-components=1 --exclude='{.gitignore}'"
  fi
}
...
fn_bootstrap() {
  ...
  case "$1" in
    "install")
      ...
      # fn_print_info "Download \".dotfiles\" from Github if not downloaded yet.."
      # if [ -d "$dot_directory" ] && ! grep -R "bymathias" "$dot_directory/.git/config"; then
      #   fn_print_error ""
      #   echo "not my dotfiles"
      # else
      #   echo "my dotfiles"
      # fi
    ...
    "test"|*)
      ...
      fn_download "$git_repository_tarball" "$dot_directory-test"
